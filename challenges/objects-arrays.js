...
let universities = [];
graduates.map(item => {
 return universities.push( `University: ${item.university}`);
})
...
console.log(unisWithUni, 'unis with unis');
...
zooAnimals.map(item => {
  return lowCaseAnimalNames.push(`ANIMAL NAMES: ${item.animal_name.toLowerCase()}`)
})
...
zooAnimals.filter(item => {
  if(item.population <= 4) {
    return lowPopulationAnimals.push(`Population animals less than 5: ${item.animal_name}`)
  }
})
...
const populationTotal = zooAnimals.reduce((total, counter) => {
  return total + counter.population
},0)
